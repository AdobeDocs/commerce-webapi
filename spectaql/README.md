...
| File | Role |
|---|---|
| `config_2-4-9.yml` | SpectaQL config for the 2.4.9 schema (`schema_2-4-9.json` → split chunk files). |
...
1. The `<Fragment>` block in the corresponding `src/pages/reference/graphql/*/index.md` is rewritten to reference exactly the generated chunk files.
...
1. Review the updated chunk files in `src/pages/includes/autogenerated/` and the updated `index.md` in `src/pages/reference/graphql/X-Y-Z/`.
