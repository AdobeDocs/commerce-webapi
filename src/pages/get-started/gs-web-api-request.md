...
| [SaaS only](https://experienceleague.adobe.com/en/docs/commerce/user-guides/product-solutions) `Store` | The store view code on which to perform the request. The value can be `all`, `default`, or the code that is defined when a store view is created. In Adobe Commerce as a Cloud Service, this is the only way to specify a store view code in a REST call. | `<store_view_code>` |
...
The call payload is a set of input *parameters* and *attributes* that you supply with the request. API operations have both _required_ and _optional_ inputs.
...
1. Set the headers, URI and method to a request object. Use URI `/V1/customers/search` and method `GET` values. Use the `searchCriteria` parameter to complete the Customer Search query. See [searchCriteria usage](/rest/use-rest/performing-searches.md).
