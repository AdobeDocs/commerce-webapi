...
When a merchant creates and activates an integration, Magento generates a consumer key, consumer secret, access token, and access token secret. All of these entities are used for [OAuth-based authentication](./authentication/gs-authentication-oauth.html).
...
If you are trying to upgrade from a previous version and need to update your integration implementation to properly utilize the OAuth workflow, review [OAuth-based Authentication](./authentication/gs-authentication-oauth.html). Otherwise, you can partially update your integration to simply store and utilize all four credentials to sign your requests.
...
For more information about the `curl` command, see [Use cURL to run the request](./gs-curl.html)
...
[Construct a request](./gs-web-api-request.html)
