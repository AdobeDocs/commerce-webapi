...
// Re-point bare `(#anchor)` links to `pageFile#anchor` when their target
// lives on a different page than the link itself. The link text reliably
// names the target (SpectaQL always links a field/argument to its own type),
// so matching it exactly against the case-sensitive heading map resolves the
// target unambiguously — no need to guess from the anchor text alone.
...
function rewriteBareAnchors(content, currentPageFile, headingToPage) {
  return content.replace(/\[((?:[^\[\]\n]|\[[^\[\]\n]*\])*)\]\(#([a-z0-9_-]+)\)/g, (fullMatch, label, anchor) => {
    ...
    return `[${label}](${targetPage}#${anchor})`;
  });
}
...
for (const schema of toRun) {
  ...
  // Queries file: preamble (endpoint/header boilerplate) + queries section.
  {
    ...
    const body = rewriteBareAnchors(queriesBody, 'index.md', headingToPage);
  ...
  if (mutationsBody) {
    ...
    const body = rewriteBareAnchors(mutationsBody, 'mutations.md', headingToPage);
    fs.writeFileSync(path.join(outputDir, fragmentFile), body, 'utf8');
    console.log(`  wrote ${fragmentFile}`);
    pageSpecs.push({
  ...
  if (subscriptionsBody) {
    ...
    const body = rewriteBareAnchors(subscriptionsBody, 'subscriptions.md', headingToPage);
    fs.writeFileSync(path.join(outputDir, fragmentFile), body, 'utf8');
    console.log(`  wrote ${fragmentFile}`);
    pageSpecs.push({
  ...
  for (const chunk of typeChunks) {
    ...
    const body = rewriteBareAnchors(chunk.content, pageFile, headingToPage);
    fs.writeFileSync(path.join(outputDir, fragmentFile), body, 'utf8');
    console.log(`  wrote ${fragmentFile}`);
    pageSpecs.push({
      pageFile,
