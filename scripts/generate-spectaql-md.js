...
function updateIndexFragments(indexPath, fragmentFiles) {
  ...
  const fragmentBlock = fragmentFiles
    .map(f => `<Fragment src="../../../includes/autogenerated/${f}" />`)
    .join('\n\n');
...
const schemas = [
  { version: '2.4.9', config: 'spectaql/config_2-4-9.yml', indexDir: 'src/pages/reference/graphql/2-4-9' },
...
for (const { config, indexDir } of toRun) {
  ...
  updateIndexFragments(indexPath, generatedFiles);
