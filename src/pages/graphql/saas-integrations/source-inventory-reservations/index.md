...
Source-level inventory reservations let a shopper or a backend flow commit a cart line to a specific inventory source before checkout, such as a store for pickup, a preferred warehouse, or a ship-from-store location. Use this feature to build buy online, pick up in store (BOPIS) and other nominated-source experiences.

Native Inventory Management holds stock at the stock level and selects a source only at shipment. Source-level reservations keep the inventory hold, the order placement guard, and the physical deduction aligned to the nominated source. Cart items without a nomination are unaffected. They continue to use native Inventory Management, which creates a stock-level reservation and selects a source at shipment through the source selection algorithm.

Storefront interaction uses one mutation, one query, and two cart-item fields:

- [`setNominatedSourceOnCartItems`](#nominate-a-source-on-cart-items) sets or clears the nominated source on cart items.
- [`sourceAvailability`](#read-availability-and-saleability) reports saleability per SKU and availability per source.
- [`nominated_source` and `nominated_source_errors`](#read-nomination-fields-on-cart-items) report the live state of a nomination on any cart item.

## Configure the storefront query

Two settings gate the feature on the storefront. Both are secure by default, so nothing about your inventory sources is disclosed until you opt in.
...
While this setting is disabled, the `sources` list in the query response is empty. The SKU-level `is_saleable` and `backorder_message` values are still returned, so the Add to Cart decision never depends on this setting.

For more information, see [Global options](https://experienceleague.adobe.com/en/docs/commerce-admin/inventory/configuration/global-options) and [Inventory configuration](https://experienceleague.adobe.com/en/docs/commerce-admin/config/catalog/inventory#per-source-availability-storefront) in the _Adobe Commerce Admin Systems Guide_.
...
A source is reported on the storefront only when it is flagged as visible on the storefront, assigned to the stock of the current sales channel, and enabled. Nomination at checkout does not require the flag. The flag controls only what the read query discloses.
...
After orders begin to carry nominations, operators can review each nomination and receive early validation in the following Admin surfaces:
...
- **Ship** > **Source Selection**. On a multi-source order, each nominated line displays its nominated source, so the operator ships the item from the correct location.
...
Source Selection validation is an operator convenience. Authoritative shipment enforcement occurs outside the Admin and rejects a deduction from the wrong source for non-Admin callers, such as REST clients and asynchronous consumers.
...
When a nomination fails domain validation, the item is returned in `rejected_items` and the remaining nominations in the request are applied. The following response shows one item that is short at the selected source.
...
The `sourceAvailability` query returns a SKU-level saleability verdict for each requested SKU. When per-source disclosure is enabled, the query also returns per-source availability, which is calculated the same way as the order placement guard, as physical quantity net of open source-level reservations.

A request accepts a maximum of 100 `skus` values and, when supplied, a maximum of 100 `source_codes` values. Split larger lists into batches that stay within both limits.
...
The per-source `is_in_stock` field is a physical-inventory statement about a single source. It can be `false` for a SKU that remains purchasable through backorder, so gating the Add to Cart button on it incorrectly hides pre-order products. The `is_saleable` field is returned for every SKU, whether or not per-source disclosure is enabled for the store.

### Identify pre-order products

The `backorder_message` field is populated only when the SKU is saleable solely because notify-mode backorders are enabled and the salable quantity is depleted, net of open reservations and the out-of-stock threshold. The field is `null` when salable quantity remains, when backorders are silent, or when the SKU is not saleable. Use the presence of a message to render a pre-order label next to the Add to Cart button.

### Interpret available_qty

The `available_qty` field returns an exact quantity only when the quantity is at or below the **Only X left Threshold** setting. Above the threshold, the field returns `null`. The threshold defaults to `0`, so by default every positive quantity is masked and the boolean fields carry the meaning.

### Control source visibility

The `sources` list is populated only when per-source disclosure is enabled, and it is narrowed to the storefront-visible sources in the stock of the current sales channel. When disclosure is disabled, the list is empty, but `is_saleable` and `backorder_message` are still returned.
...
After a cart that contains nominations is placed, the reservation lifecycle runs automatically.
...
1. **Cleanup.** A daily cron job prunes fully compensated rows from the append-only reservation ledger, so it does not grow without bound. No manual step is required.
